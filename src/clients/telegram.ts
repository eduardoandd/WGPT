...
export class TelegramAdapter implements ClientAdapter {
    ...
    async start(onMessage: (msg: IncomingMessage) => Promise<void>): Promise<void> {
        ...
        this.bot.on(message('text'), async (ctx) => {
            if (ctx.chat.type !== 'private') return; // Ignorar grupos
        ...
        this.bot.on(message('document'), async (ctx) => {
            ...
            if (isPdf || isSpreadsheet) {
                ...
                try {
                    // Telegram entrega uma URL temporária para download do arquivo
        ...
        // Parar o bot limpo quando o processo encerrar
    ...
    async sendText(chatId: string, text: string): Promise<void> {
        ...
        // Quebrar em chunks se necessário
